...
delete from veiculo where idVeiculo = 6

SELECT * FROM ALUGUEL A LEFT JOIN CLIENTE ON A.idcliente = CLIENTE.idCliente LEFT JOIN VEICULO ON A.idVeiculo = VEICULO.idVeiculo LEFT JOIN MODELO M ON VEICULO.idModelo = M.idModelo LEFT JOIN EMPRESA ON VEICULO.idEmpresa = EMPRESA.idEmpresa LEFT JOIN MARCA ON M.idMarca = MARCA.idMarca
